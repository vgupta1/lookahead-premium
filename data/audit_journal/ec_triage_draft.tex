...
split is visible rather than silently resolved. Re-running the classifier on one hundred records
reproduced the assigned bucket in \(99\) cases. That figure concerns decoding variation alone, and we
are explicit below that it is not the quantity a reader should care most about.
...
human override is visible as a difference between two files. No record belonging to the seed frame --
...
excluded records and classified them by hand from title and abstract alone, with the model's bucket,
rationale and confidence withheld so that the reading was independent of them. All \(50\) were
...
the \(221\) excluded records are wrongly excluded. (The corresponding Clopper--Pearson bound, which
discards the finite-population information, is \(5.8\%\), or \(12.9\) records.)
...
prompt that preserve the rule, and the choice of model is not innocuous -- an earlier execution under
a smaller model of the same family differed on \(23\) of the \(379\) records, retaining fewer of
them. Perhaps the most honest statement is that this stage is designed so that its failures are
recoverable, not so that it is correct: every subsequent stage of the audit works from full texts,
and the screening log records a ground for each exclusion so that a reader may disagree with any of
them individually.
