...
Reads ``data/audit_workshop/audit.csv`` -- one row per paper -- and emits ``outputs/tab_litaudit.tex``.
...
``data/audit_workshop/evidence.csv`` (one row per supporting passage, with file and page);
the vocabularies and inclusion rules are in ``data/audit_workshop/SCHEMA.md``.
...
AUDIT = os.path.join(ROOT, "data", "audit_workshop", "audit.csv")
...
def build(t, path=None):
    ...
    lines = [
        ...
        "% GENERATED by code/literature_audit.py from data/audit_workshop/audit.csv.",
        "% Do not hand-edit; evidence per cell in data/audit_workshop/evidence.csv.",
